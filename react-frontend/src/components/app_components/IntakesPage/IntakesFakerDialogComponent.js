...
import intakesFakerFactory from "./intakesFakerFactory";
...
const IntakesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = intakesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("intakes").create(elem));
...
export default connect(mapState, mapDispatch)(IntakesFakerDialogComponent);
