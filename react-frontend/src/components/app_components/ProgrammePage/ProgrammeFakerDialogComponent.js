...
import programmeFakerFactory from "./programmeFakerFactory";
...
const ProgrammeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = programmeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("programme").create(elem));
...
export default connect(mapState, mapDispatch)(ProgrammeFakerDialogComponent);
